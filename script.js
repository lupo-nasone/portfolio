...
function discord(){
    movimento()
    gsap.to("#nuovoimg", {duration: 0.5, opacity: 0});
    
    function cambiaimg(){
        document.getElementById("nuovoimg").src ="images/discord.png";
    }
    setTimeout(cambiaimg, 500);
    gsap.to("#nuovoimg", {duration: 0.5, opacity: 1, delay: 0.5});
...
function movimento(){
    if(n){
        gsap.to(".socialedes", {duration: 1, x:"800px", rotate: "360deg"});
    }
    if(!n){
        gsap.to(".socialedes", {duration: 1, x:0, rotate: "-360deg"});
    }
    n=!n;
}